...
const screenshots = [
  ...
  { src: "/images/pm-quiz-question.png", label: "Quiz Interface" },
  { src: "/images/pm-quiz-results.png", label: "Results Dashboard" },
];
    ...
    <>
      <div className="grid gap-4 sm:grid-cols-3">
        {screenshots.map(({ src, label }) => (
          ...
          >
            ...
            <p className="px-3 py-2.5 text-center text-xs font-medium text-muted-foreground">
              {label}
            </p>
          </button>
        ))}
      ...
      {openImg && (
        ...
        >
          ...
          <p className="absolute bottom-5 left-1/2 -translate-x-1/2 rounded-full bg-white/10 px-4 py-1.5 text-xs font-medium text-white/90 backdrop-blur-sm">
            {openImg.label}
          </p>
        </div>
      )}
